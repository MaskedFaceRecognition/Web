...
@app.route("/")
def hello2():
    angry_path = '/home/ubuntu/graduation-project/Web/static/Emotion/angry'
    happy_path = '/home/ubuntu/graduation-project/Web/static/Emotion/happy'
    neutral_path = '/home/ubuntu/graduation-project/Web/static/Emotion/neutral'
...
def multi_upload_emotion():
    ...
    for file in uploaded_files:
        file.save("/home/ubuntu/graduation-project/Web/static/Emotion/{}.jpg".format(IDX))
        IDX += 1
...
def multi_upload_gan():
    ...
    IDX = 0
    for file in uploaded_files:
        file.save("/home/ubuntu/graduation-project/Web/static/GAN/{}.jpg".format(IDX))
        IDX += 1
    return redirect(url_for("hello2"))
...
def emotion_prediction():
    ...
    index_angry = 0
    index_neutral = 0
    model = load_model('/home/ubuntu/graduation-project/Web/models/model_best_0_2.h5')
    for file in uploaded_files:
        filestr = file.read() # byte 단위이기 때문에 바로 file.save로 저장해서 .jpg로 보이지 않는다.
        
        detection_model_path = '/home/ubuntu/graduation-project/Web/models/haarcascade_frontalface_default.xml'
        ...
        if prediction[0][0] >= 0.5:
            cv2.imwrite("/home/ubuntu/graduation-project/Web/static/Emotion/angry/{}.jpg".format(index_angry), saveFile) # numpy.ndarray
            index_angry += 1
        elif prediction[0][1] >= 0.5:
            cv2.imwrite("/home/ubuntu/graduation-project/Web/static/Emotion/happy/{}.jpg".format(index_happy), saveFile)
            index_happy += 1
        elif prediction[0][2] >= 0.5:
            cv2.imwrite("/home/ubuntu/graduation-project/Web/static/Emotion/neutral/{}.jpg".format(index_neutral), saveFile)
            index_neutral += 1
    return redirect(url_for("hello2"))
...
def upload_test():
    ...
    IDX = 0
    for file in uploaded_files:
        file.save("/home/ubuntu/graduation-project/Web/static/test/{}.jpg".format(IDX))
        IDX += 1
...
def train_angry():
    train_photo_path = '/home/ubuntu/graduation-project/Web/static/Emotion/angry/*'
    save_path = '/home/ubuntu/graduation-project/Web/static/npy/npy_angry/'
    test_photo_path = '/home/ubuntu/graduation-project/Web/static/test/*'
    to_npy(train_photo_path,test_photo_path,save_path)
    train(save_path)

@app.route('/train_happy',methods = ["POST"])
def train_happy():
    train_photo_path = '/home/ubuntu/graduation-project/Web/static/Emotion/happy/*'
    save_path = '/home/ubuntu/graduation-project/Web/static/npy/npy_happy/'
    test_photo_path = '/home/ubuntu/graduation-project/Web/static/test/*'
    to_npy(train_photo_path,test_photo_path,save_path)
    train(save_path)
...
def train_neutral():
    train_photo_path = '/home/ubuntu/graduation-project/Web/static/Emotion/neutral/*'
    save_path = '/home/ubuntu/graduation-project/Web/static/npy/npy_neutral/'
    test_photo_path = '/home/ubuntu/graduation-project/Web/static/test/*'
    to_npy(train_photo_path,test_photo_path,save_path)
    train(save_path)
...
def train(npy_path):
    ...
    if tf.train.get_checkpoint_state('/home/ubuntu/graduation-project/Web/src_upgraded/backup'):
        #saver = tf.compat.v1.train.Saver()
        saver.restore(sess, '/home/ubuntu/graduation-project/Web/src_upgraded/backup/latest')
    ...
    while True:
        ...
        args = {'face': '/home/ubuntu/graduation-project/Web/src_upgraded/face_detector', 'model': '/home/ubuntu/graduation-project/Web/src_upgraded/mask_detector.model', 'confidence': 0.5}
        #print(f'args : {args}') ###
        ...
        if sess.run(epoch) <= PRETRAIN_EPOCH:
            ...
            cv2.imwrite('/home/ubuntu/graduation-project/Web/static/output/{}.jpg'.format("{0:06d}".format(sess.run(epoch))), cv2.cvtColor(sample, cv2.COLOR_RGB2BGR))
            saver.save(sess, '/home/ubuntu/graduation-project/Web/src_upgraded/backup/latest', write_meta_graph=False)
            if sess.run(epoch) == PRETRAIN_EPOCH:
                saver.save(sess, '/home/ubuntu/graduation-project/Web/src_upgraded/backup/pretrained', write_meta_graph=False)
        # Discrimitation
        else:
            g_loss_value = 0
            d_loss_value = 0
            ...
            completion = sess.run(model.completion, feed_dict={x: x_batch, mask: mask_batch, is_training: False})
            sample = np.array((completion[0] + 1) * 127.5, dtype=np.uint8)
            cv2.imwrite('/home/ubuntu/graduation-project/Web/src_upgraded/output/{}.jpg'.format("{0:06d}".format(sess.run(epoch))), cv2.cvtColor(sample, cv2.COLOR_RGB2BGR))
